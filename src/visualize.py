...
def fig_quality_norm(qn):
    ...
    fig.suptitle("JGraphT — QMOOD Kalite Niteliklerinin Surumlere Gore Evrimi",
                 fontsize=13)
...
def fig_design_size(dm):
    ...
    plt.title("JGraphT — Tasarim Boyutu Buyumesi (Design Size & Complexity)")
    fig.tight_layout()
...
def fig_ck_means(ck):
    ...
    plt.title("JGraphT — CK Metrik Ortalamalarinin Evrimi")
    plt.legend(ncol=3)
    plt.grid(alpha=.3)
    plt.tight_layout()
...
def fig_coupling_cohesion(dm):
    ...
    plt.title("JGraphT — Coupling (DCC) vs Cohesion (CAM)")
    fig.tight_layout()
    ...
    for i in range(len(QUALITIES)):
        for j in range(len(qn)):
            plt.text(j, i, f"{data[i, j]:.2f}", ha="center", va="center",
                     fontsize=7)
    plt.title("JGraphT — QMOOD Kalite Nitelikleri Isi Haritasi (normalize)")
    plt.tight_layout()
...
def fig_radar(dm):
    ...
    ax.set_title("JGraphT — Tasarim Metrikleri: ilk vs son surum (normalize)")
