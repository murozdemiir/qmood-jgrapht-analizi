...
def main():
    # Ana karsilastirma 01 (tam evrim) promptunun yanitlari uzerinden yapilir;
    # her model tek bir 01 yaniti uretmistir (02/03 promptlari tamamlayicidir).
    files = sorted(glob.glob(os.path.join(RESP, "01_*.md")))
    if not files:
        print("[!] llm_responses/ altinda 01_*.md yaniti bulunamadi.")
        print("    Once promptlari modellere verip yanitlari kaydedin.")
        return
    rows = []
    for f in files:
        # 01_llm_prompt_<model>.md -> <model>
        name = os.path.splitext(os.path.basename(f))[0]
        model = name.replace("01_llm_prompt_", "")
