...
GOREV: Sana acik kaynak 'JGraphT' (Java graf teorisi kutuphanesi: graf veri
yapilari ve algoritmalari, jgrapht-core modulu) kutuphanesinin farkli
...
def main():
    ...
    for v in versions[-3:]:
        ...
        with open(os.path.join(OUT, f"02_version_{v}_prompt.txt"), "w", encoding="utf-8") as f:
            f.write(SYSTEM_PROMPT + "\n")
            f.write(f"TEK SURUM DERIN INCELEME — JGraphT {v}\n")
            f.write("-" * 50 + "\n")
    ...
    for a, b in pairs:
        ...
        with open(os.path.join(OUT, f"03_diff_{a}_to_{b}_prompt.txt"), "w", encoding="utf-8") as f:
            f.write(SYSTEM_PROMPT + "\n")
            f.write(f"SURUM FARKI ANALIZI — JGraphT {a} -> {b}\n")
            f.write("-" * 50 + "\n")
